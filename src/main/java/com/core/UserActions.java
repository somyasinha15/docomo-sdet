...
public class UserActions<T> extends DriverManager<T> {
    ...
    public static Map<String, String> get(Map<String, String> formParams) {
        return formParams.entrySet().stream().filter(entry -> entry.getValue() != null)
    ...
    private void fluentWait(WebElement element, int timeout) {
        try {
            Wait wait = new FluentWait<>(driverThread).withTimeout(Duration.ofSeconds(timeout))
    ...
    protected void waitForElements(List<WebElement> elements) {
        new WebDriverWait(driverThread, Duration.ofSeconds(30))
    ...
    private void loading(WebElement loadingElement) {
        Awaitility.await("Wait for new user to load").atMost(5, TimeUnit.SECONDS).until(loadingElement::getText,
                not("loading..."));
    }
    ...
    private WebElement findElement(final By locator, final int timeoutSeconds) {
        FluentWait<WebDriver> wait = new FluentWait<>(driverThread).withTimeout(Duration.ofSeconds(timeoutSeconds))
                .pollingEvery(Duration.ofMillis(100)).withMessage("Timeout occured!")
                ...
                if (t_testcasename.equalsIgnoreCase(p_testcaseName)
                    ...
                    for (i = 1; i < csvobj.getColumnCount() / 2 + 1; i++) {
                        ...
                        if (p_filed.equalsIgnoreCase(t_field)) {
                            ...
                            String stp = csvOutput.replace(FileContentPerRow, t_field + "," + p_field1,
                                    t_field + "," + t_value);
                    ...
                    if (P_valuenotduplicated == 0) {
                        ...
                        dicttoread.put(t_field, t_value);
                        String stp1 = csvOutput.replace(FileContentPerRow, p_field1,
                                p_field1 + "," + t_field + "," + t_value);
        ...
        try {
            if (!SystemUtils.IS_OS_LINUX || !SystemUtils.IS_OS_MAC_OSX) {
                ...
                FileUtils.copyFile(src,
                        new File(("ScreensDoc\\" + p_testcaseName + "\\" + datetimeabc + "\\" + Counter + ".png")));
            }
        } catch (Exception e) {
        }
    }
            ...
            try (XWPFDocument doc = new XWPFDocument()) {
                ...
                for (int i = 1; i <= Counter; i++) {
                    ...
                    try (FileInputStream pic = new FileInputStream(path)) {
                        r.addBreak();
                        r.addCarriageReturn();
                        r.addPicture(pic, XWPFDocument.PICTURE_TYPE_PNG,
                                "ScreensDoc\\" + p_testcaseName1 + "\\" + datetimeabc + "\\" + i + ".png",
                                Units.toEMU(300), Units.toEMU(400));
                        FileOutputStream out = new FileOutputStream("ScreensDoc\\" + p_testcaseName1 + "\\"
                                + datetimeabc + "\\" + p_testcaseName1 + ".docx");
                        doc.write(out);
                        pic.close();
                        out.close();
