...
public class WebDriverController<T> extends DriverOptions<T> {
    ...
    private synchronized void initDriver(String browser, String grid, String perf) {
        ...
        CreateTestGridUrlRequest request = CreateTestGridUrlRequest.builder().expiresInSeconds(300).projectArn(
                "arn:aws:devicefarm:ap-southeast-2:111122223333:testgrid-project:1111111-2222-3333-4444-555555555")
                .build();
        try {
            switch (grid) {
            case "CLOUD":
                logger.info(
                        "Make sure that the environment variables AWS_ACCESS_KEY and AWS_SECRET_KEY are configured in your testing environment.");
            ...
            case "DOCKER":
                ...
                _driverThread = new RemoteWebDriver(URI.create("http://localhost:4444/").toURL(),
                        (Capabilities) getBrowserOptions(browser, perf));
                logger.info("Grid client setup for Docker containers successful");
                break;
            case "BROWSERSTACK":
                logger.info("Make sure that browserstack configs provided");
                _driverThread = new RemoteWebDriver(
                        new URL("https://" + username + ":" + accessKey + "@hub-cloud.browserstack.com/wd/hub"),
                        addBrowserStackCapabilities(browser, testName));
